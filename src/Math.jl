...
function _parse_SBML_math!(math_idents::Vector{String}, math::SBML.MathApply;
    ...
    if math.fn ∈ ["lt", "gt", "leq", "geq", "eq", "neq"] && inequality_to_julia == true
        if length(math.args) != 2
            throw(SBMLSupport("lt, gt, leq, geq with more than 2 arguments are not supported"))
        end
        part1, _ = _parse_SBML_math!(math_idents, math.args[1]) 
    ...
    # We currently do not support SBML models with delay 
    if math.fn == "delay"
        throw(SBMLSupport("SBML models with delay are not supported"))
    end

    # We currently do not support SBML models with implies
    if math.fn == "implies"
        throw(SBMLSupport("SBML models with implies are not supported"))
    end
