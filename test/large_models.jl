...
b1 = @elapsed rn, cb = load_SBML(path; inline_assignment_rules = true)
...
b2 = @elapsed rn, cb = load_SBML(path; inline_assignment_rules = true)
@test b2 ≤ 500

# Test that SBMLImporter correctly enforces mass action
path = joinpath(@__DIR__, "Models", "egfr_net.xml")
b3 = @elapsed rn, cb = load_SBML(path; massaction = true, inline_assignment_rules = true)
