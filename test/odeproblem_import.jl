...
function test_odeproblem_import(path_SBML; atol = 1.0e-6)::Nothing
    model_SBML = SBMLImporter.parse_SBML(
        path_SBML, false; model_as_string = false, inline_assignment_rules = true
    )
    ...
    sol2 = solve(
        oprob2, Rodas5P(), abstol = 1.0e-10, reltol = 1.0e-10, saveat = 1:10,
        callback = cb2
    )
...
models = [
    "brusselator_kinetic_parameters.xml", "model_Boehm_JProteomeRes2014.xml",
